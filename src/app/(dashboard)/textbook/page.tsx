...
import { Plus, Camera, ChevronRight, ChevronDown, Search, FolderOpen, X, BookOpen, Trash2, Upload } from 'lucide-react'
        ...
        {subjects.map(subject => {
          ...
          return (
            <div key={subject.id} style={{background:'white',border:'1px solid #e2e8f0',borderRadius:'14px',overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
              <button onClick={() => toggle(subject.name)} style={{width:'100%',display:'flex',alignItems:'center',gap:'12px',padding:'14px 16px',background:'none',border:'none',cursor:'pointer',transition:'background 0.15s'}}
              ...
              {isExpanded && (
                <div style={{borderTop:'1px solid #f1f5f9'}}>
                  {sBooks.length === 0 ? (
                    ...
                    sBooks.map(book => (
                      <div key={book.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 16px',borderBottom:'1px solid #f8fafc',cursor:'pointer',transition:'background 0.15s'}}
                        onMouseEnter={e=>(e.currentTarget as any).style.background='#f8fafc'}
                        onMouseLeave={e=>(e.currentTarget as any).style.background='none'}
            ...
            {uploading ? (
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'10px'}}>
                {[
                  ...
                  <button key={btn.label} onClick={btn.action} style={{background:btn.bg,border:`1.5px solid ${btn.border}`,borderRadius:'12px',padding:'16px 8px',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px',cursor:'pointer',transition:'transform 0.15s'}}
